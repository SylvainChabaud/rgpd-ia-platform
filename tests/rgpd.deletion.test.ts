...
async function cleanup() {
  ...
  await pool.query("DELETE FROM rgpd_requests WHERE tenant_id IN ($1, $2)", [
    TENANT_A_ID,
    TENANT_B_ID,
  ]);
  await pool.query("DELETE FROM audit_events WHERE tenant_id IN ($1, $2)", [
    TENANT_A_ID,
    TENANT_B_ID,
  ]);
...
describe("LOT 5.2 - RGPD Deletion (BLOCKER)", () => {
  ...
  test("BLOCKER: Soft delete is tenant-scoped (no cross-tenant deletion)", async () => {
    ...
    const userBBefore = await pool.query(
    ...
    const userBAfter = await pool.query(
      "SELECT id, deleted_at FROM users WHERE id = $1",
      [USER_B_ID]
    );
  ...
  test("BLOCKER: Purge performs hard delete (data irrecoverable)", async () => {
    ...
    const jobsRes = await pool.query("SELECT * FROM ai_jobs WHERE user_id = $1", [
      freshUserId,
    ]);
